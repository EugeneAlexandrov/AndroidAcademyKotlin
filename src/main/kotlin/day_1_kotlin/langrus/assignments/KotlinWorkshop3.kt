...
object KotlinWorkshop3 {
    ...
    private fun printGameStats(guesses: IntArray, guessCounter: Int, randomNumber: Int) {
        ...
        println("Total count of guesses higher than randomNumber: ${guesses.countHigherGuesses(guesses, randomNumber)}")
    ...
    private fun IntArray.countHigherGuesses(guesses: IntArray, randomNumber: Int): Int {
        return this.filter { it > randomNumber }.count()
    }
    ...
    val countLowerGuesses: (IntArray, Int) -> Unit = { guesses, randomNumber ->
        ...
        for (guess in guesses) {
            if (guess != -1 && guess < randomNumber) {
                println("$guess меньше $randomNumber")
                counter++
            }
        }
        println("Total count of guess less then randomnumber: $counter")
    }
